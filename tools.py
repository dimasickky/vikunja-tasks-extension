...
the the backend service HTTP client in ``app`` — business logic lives here,
...
class TasksExtension(Extension):
    ...
    Backend: the backend service (the backend host:8102) → Vikunja (the backend host:3456).
    ...
    def _uid(ctx) -> str:
        ...
        if not uid:
            raise RuntimeError(
                ...
                "provisioned imperal_id to talk to the backend service.",
            )
        return uid
